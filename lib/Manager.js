...
class Manager {
	constructor() {
		...
		chrome.notifications.onButtonClicked.addListener(function (notificationId, btnIdx) {
			const notification = m.current[notificationId];
			if (notification) {
				notification.emit('buttonClicked', btnIdx);
			}
		});
		chrome.notifications.onClicked.addListener((notificationId) => {
			...
			if (notification) {
				...
				delete m.current[notificationId];
				Utility.clear(notificationId)
					.then(() => notification.emit('closed', CLOSED_BY_CLICK))
					.catch(err => m.logger.error('Clear notification error', err));
			}
		});

		chrome.notifications.onClosed.addListener((notificationId, byUser) => {
			m.logger.info(`Notification ${notificationId} closed`);
			const notification = m.current[notificationId];

			//allow time for buttonClicked to send the event
			setTimeout(function () {
				if (notification) {
					delete m.current[notificationId];
			...
			.then((count) => {
				...
				return Utility.create(id, notification.options)
					.then(() => {
						...
						const displayTime = notification.displayTime || this.displayTime;
